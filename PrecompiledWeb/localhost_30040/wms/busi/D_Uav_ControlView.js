...
(function () {
    ...
    var createShipWinContent = function () {
        return '<div class="locusBox">'
            ...
            + '<a href="javascript:void(0)" id="btnNewShipTrack">船舶跟踪</a>'
            + '</div>'
            + '</div>'
            + '</div>';
    };
    ...
    var addShipWinBtnEvent = function () {
        ...
        //创建新目标 船舶跟踪任务
        util.addEvent(util.getElement('btnNewShipTrack', parent), 'click', function (event) {
            util.stopPropagation(event);

            var ShipInfo = mini.encode(shipInfo);
            shipWin.hide();
            //var shipName = _Data.name;//船名
            //var mmsi = _Data.MMSI;
            //var latFormat = isNaN(_Data.lat) ? '' : util.formatLat(_Data.lat);//格式化纬度
            //var lngFormat = isNaN(_Data.lng) ? '' : util.formatLng(_Data.lng);

            var lat = _DataControl.lat;//纬度未格式化
            var lng = _DataControl.lng;
            ...
            bodyLoading();
            $.ajax({
                url: _Datapath + "Busi/DUavMainTaskCode.aspx?method=onCreateTrackLine",
                data: { lng: lng, lat: lat, TaskId: row.BILLID, UavID: row.UavID, LineHeight: row.LineHeight, ShipInfo: ShipInfo },
                type: "post",
                success: function (text) {
                    ...
                    if (data.IsOK) {
                        ...
                        if (data.DataList != null && data.DataList.length >= 1) {
                            var trackLine = data.DataList;

                            for (var i = 0; i < trackLine.length; i++) {
                                joinLine(i, trackLine[i].Wd, trackLine[i].Jd);
                            }
