...
var CopyExcel = function (grid) {
    ...
    var me = this;
    me.grid = grid;
    me.copyAlldata = function (grid) {
        var valArray = getAllValueArrayFromGrid(grid);
    ...
    /////////////////////////////////////////////////////////////////////////
	    ...
	    function getAllValueArrayFromGrid(grid) {
		...
		for(var i=1,l=columns.length;i<l;i++){
			heardArray.push(grid.columns[i].header);
		}
		array.push(heardArray);
            for (var i = 0, l = grid.data.length; i < l; i++) {
                var textArray = [];
                array.push(textArray);

                var row = grid.getAt(i);
                for (var j = 1, k = columns.length; j < k; j++) {
